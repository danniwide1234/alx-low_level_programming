...
int shash_table_set(shash_table_t *ht, const char *key, const char *value)
{
	...
	while (storage)
	{
		...
		{
			free(storage->value);
			storage->value = value_copy;
			return (1);
		}
		storage = storage->snext;
	}
	...
	{
		storage = ht->shead;
		while (storage->snext != NULL && strcmp(storage->snext->key, key) < 0)
			storage = storage->snext;
		new->sprev = storage;
		new->snext = storage->snext;
		if (storage->snext == NULL)
			ht->stail = new;
		else
			storage->snext->sprev = new;
		storage->snext = new;
	}
